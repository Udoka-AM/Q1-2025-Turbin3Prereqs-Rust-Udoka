...
mod tests {
    ...
    #[test]
    fn airdrop() {
        let keypair = read_keypair_file("dev-wallet.json").expect("Failed to read wallet file");
    ...
    fn transfer_sol() {
        ...
        let to_pubkey = Pubkey::from_str("HBgGPNgPrTdcj1GBfhq23k8VHT1pZskKADqJq6K2EydA")
    ...
    fn complete_prerequisites() -> Result<(), Box<dyn std::error::Error>> {
        let signer = read_keypair_file("turbin3-wallet.json").expect("Failed to read wallet file");
        ...
        match client.get_account(&prereq) {
            Err(_) => {
                ...
                let args = CompleteArgs {
                    github: "Udoka-AM".as_bytes().to_vec(),
                };
